 ...
 *              }
 *              
 *              ** catch => 에러가 발생시 처리하는 영억
 *                 ----- 다중 catch가 가능
 *                 ----- 에러 종류에 따라 딸로 잡는 경우도 있다
 *                 ----- 한게 통합(최상의 예외 클래스를 이용한다)
 *                       ------- Exception / Throwable
 *                               ---------   ---------
 *                                예외만 가능    예외 + 처리
 ...
 *                          SQLEception                      |
 *                          Class NotFoundException    NullPointException
 *                                                     NumberFormatException
 *                                                     ...
 *             *** try에서 정상수행을 하면 => catch는 수행하지 않는다
 *                 ------------- finally가 있는 경우에는 finally는 수행
 *             *** try에 있는 문자을 수행하는 과정에서 에러가 난 경우에만
 *                 catch 수행
 *             예)
 *                 문장 1
 *                 문장 2
...
public class 예외처리_예외복구 {
	...
	public static void main(String[] args) throws Exception{
		...
		// 인스턴스는 사용이 불가능 = > 사용시에는 반드시 객체생성 후 사용
		// static은 바로 사용이 가능 => 메모리에 저장되어서 바로 사용이 가능
