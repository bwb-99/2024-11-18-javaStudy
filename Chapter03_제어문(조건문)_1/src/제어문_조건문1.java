 ...
 * 					  => 오류 방지
 * 						 ======= if으로 처리가 어려운 과정 : 예외처리
 * 					  => 사전에 방지 => if => 이런 경우
 * 
 * 			 단일 조건문
 * 			   형식)		| 연산자 => 부정연산자(!) => 비교연산자, 논리연산자 사용이 가능
 * 					if(조건문 => 반드시 true/false) 
 * 					{
 ...
 *						int a=1
 *						if문장마다 다른 문자으로 되어 있다 => 모든 if의 조건을 검색
 *						if(a==1) =>종료
 *						if(a==2)
 *						if(a==3)
 *						if(a==4)
 ...
 *
 *					=> 두개가 나눠지는 경우
 *						1. 로그인 => 로그인(main화면이동) => if 문
 *									로그인이 안된 경우(back, 회원가입) => else문
 *						2. 찜하기 / 좋아요 => if~else
 *						3. 대문자 / 소문자 , 3의 배수					
 * 			다중 조건문
 * 		= 선택문 => 게임 => 네트워크
...
// => 이미 만들어진 클래스 : 라이브러리
// 클래스를 가지고 올때 import , java.util.* => 클래스 전체를 가져오는 것이므로 속도가 느려진다. 속도 빠르게 한두개만 가져올때 Scannser 클래스 하나만
public class 제어문_조건문1 {
	...
	public static void main(String[] args) {
		...
		//if(com==0) // 컴퓨터가 가위를 낸 경우
		{
			...
			{
				if(com==0 && player==2)//가위
					System.out.println("사용자가 졌다.");
				if(com==1 && player==2)//바위
					System.out.println("사용자가 이겼다.");
				if(com==2 && player==2)//보
					System.out.println("비겼다.");
			}
